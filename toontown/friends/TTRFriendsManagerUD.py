...
class TTRFriendsManagerUD(DistributedObjectGlobalUD):
    """
    The Toontown Porkheffley Friends Manager UberDOG, or TTRFMUD for short.
    ...
    def comingOnline(self, avId, friends):
        ...
        print('Avatar Coming Online')
        channel = self.GetPuppetConnectionChannel(avId)
        dgcleanup = self.dclass.aiFormatUpdate('goingOffline', self.doId, self.doId, self.air.ourChannel, [avId])
        dg = PyDatagram()
        dg.addServerHeader(channel, self.air.ourChannel, CLIENTAGENT_ADD_POST_REMOVE)
        dg.addBlob(dgcleanup.getMessage()) # PY3
        self.air.send(dg)
